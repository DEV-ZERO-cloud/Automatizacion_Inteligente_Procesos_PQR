...
CLASSIFY_ENDPOINT  = "/classify/{pqr_id}"
...
def call_classify(host: str, pqr_id: int, token: Optional[str] = None) -> tuple[dict, float]:
    """
    POST /classify/{pqr_id} → clasifica el PQR y retorna (respuesta_json, tiempo_ms).
...
def run_batch(host: str, filepath: str, token: Optional[str] = None, clasificacion_id=None):
    ...
    Modo batch:
      ...
      3. POST /classify/{id}  → clasifica y obtiene predicción.
